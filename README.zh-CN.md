...
| Work day starts / ends at | 09:00 / 18:00 | 工作日当天的上下班时间 |

### 工时（Work hours）

每周额度始终是 7 天后重置，但这 168 小时里你并不是一直在工作。如果标记在整个一周里匀速移动，你会显得永远领先于进度，而每个周一早上又显得远远落后。

因此周进度条的标记**按工时而不是挂钟时间推进**：它表示这个窗口内的工时已经消耗了多少。按默认配置（周一至周五，09:00–18:00），一个 7 天窗口包含 45 个工时，第一个工作日结束时标记正好走到 20%。非工作时间里标记静止不动，所以无论什么时候瞟一眼面板，标记和用量的对比都是可信的。
